...
# 
# table(train_original$user_name)



# a <- grep("_belt", names(train_original)); (length(a))
# b <- grep("_arm", names(train_original)); (length(b))
# c <- grep("_dumbbell", names(train_original)); (length(c))
# d <- grep("_forearm", names(train_original)); (length(d))
# e <- names(train_original)[-c(a,b,c,d)]; (length(e))

# The dataset contains 38*4 vbles corresponding to the 4 sensors: 
# belt, arm, glove, forearm (glove) 
# plus 8 ID variables: X, user_name, raw_timestamp_part_1, raw_timestamp_part_2,
# cvtd_timestamp, new_window, num_window, classe

#--- OPTION 1: FILTER CASES CORRESPONDING TO NON NEW WINDOWS -------
# (all the test data cases correspond to NON new windows)
# Therefore, all summarising variables can be deleted

max(colSums(is.na(train_original)))
# [1] 19622

trainNNW <- train_original[train_original$new_window == "no",] #keep only non-window cases
#delete cols with NA in all cases
trainNNW <- trainNNW[,!colSums(is.na(trainNNW)) == nrow(trainNNW)] 
colSums(is.na(trainNNW))


#--- OPTION 2: INCLUDE ALL CASES (DELETING DERIVED VBLES) -------
...
# Esto no hace falta para 2016, porque han puesto una Quiz con las 20 preguntas
